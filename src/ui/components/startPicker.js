...
const FALLBACK_LABEL = 'Utrecht Domplein';
const FALLBACK_LATLNG = { lat: 52.0907, lng: 5.1214 };
...
export function openStartSheet({ mode, google, onUseCurrentLocation, onCoords, onPlace }) {
  ...
  if (mode === 'google' && google) {
    ...
    function submitManual() {
      ...
      const latLng = match ? { lat: Number(match[1]), lng: Number(match[2]) } : FALLBACK_LATLNG;
      const label = match ? raw : FALLBACK_LABEL;
      onCoords({ latLng, label });
      close();
    }
    ...
    fieldWrap.append(input, submitBtn);
  }
